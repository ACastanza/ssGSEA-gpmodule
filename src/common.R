...
zip.file <- function(path.to.zip.exe, outfile, pattern, options="") {
	...
	if(isWindows) {
		...
		system(paste(path.to.zip.exe, "zip.exe -q -m ", " ", options, outfile, " ", pattern, sep=''))
	} else {
		system(paste("zip -q -m ", options, " ", outfile, " ", pattern, sep=''))
	}
}
...
unzip <- function(zip.filename, dest, options="") {
	...
	if(isWindows()) {
      ...
      cmd <- paste(unzip, " ", options, " -q '", zip.filename, "' -d ", dest, sep='')
      system(cmd)
	}
}
